...
いずれのモードでも、測定電流は INA228 モジュール自身のネジ端子台だけを通り、
**キャリア基板には流れない**（J2/J3 の pin 5-7 は無接続）。J5 は電圧測定の基準を
取るための接続で、大きな電流は流れない。
...
安定化電源(+) ──► ch0 端子台 VIN+ ─[15mΩ]─ VIN− ──► DUT(+)
安定化電源(−) ◄──────────────────────────────────── DUT(−)
      │
      └───────► J5 (GND)    ※共通接続はこの 1 点のみ
```
...
```
安定化電源(+) ──► ch0 端子台 VIN+ ─[15mΩ]─ VIN− ──► DCDC IN(+)
安定化電源(−) ◄───────────────────────────────────── DCDC IN(−)
      │
      └───────► J5 (GND)

DCDC OUT(+) ──► ch1 端子台 VIN+ ─[15mΩ]─ VIN− ──► 電子負荷(+)
DCDC OUT(−) ◄───────────────────────────────────── 電子負荷(−)
```
